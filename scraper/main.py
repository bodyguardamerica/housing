...
# Note: An earlier mitigation rotated the Passkey session after N
# consecutive no_changes runs to defeat session-bound stale caching.
# PasskeyClient.scrape() now calls reset_session() at the start of every
# scrape (matching overallcoma/genconhotels's proven pattern), which is
# strictly better — the stale-cache class of bug can't reach this point.
# The counter and rotation hack were removed.
