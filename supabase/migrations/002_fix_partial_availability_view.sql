-- Fix latest_room_availability view to include partial availability rooms
...
WHERE
    -- Show rooms with full availability OR partial availability
    rs.available_count > 0
    OR (rs.raw_block_data->>'partial_availability')::BOOLEAN = TRUE
    -- Exclude rooms that are completely sold out
    AND COALESCE((rs.raw_block_data->>'sold_out')::BOOLEAN, FALSE) = FALSE
