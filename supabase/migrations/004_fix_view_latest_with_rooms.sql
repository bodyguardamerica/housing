-- Fix view to use the latest successful scrape that has room data
-- This prevents empty scrapes from overwriting partial availability data

DROP VIEW IF EXISTS latest_room_availability;

CREATE OR REPLACE VIEW latest_room_availability AS
WITH latest_scrape AS (
    -- Get the latest successful scrape that actually found rooms
